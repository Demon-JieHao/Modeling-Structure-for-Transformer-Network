...
def cumsoftmax(x, dim=-1):
    return tf.cumsum(tf.nn.softmax(x, dim=dim), axis=dim)

class ONLSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope, default_name="onlstm_cell",
                               values=[inputs, state]):
            if not isinstance(inputs, (list, tuple)):
                inputs = [inputs]

            all_inputs = list(inputs) + [state]
            ingate = tf.nn.sigmoid(linear(all_inputs, self._num_units, True, False,
                                     scope="ingate"))
            forgetgate = tf.nn.sigmoid(linear(all_inputs, self._num_units, True, False,
                                     scope="forgetgate"))
            outgate = tf.nn.sigmoid(linear(all_inputs, self._num_units, True, False,
                                     scope="outgate"))
            cell =  tf.tanh(linear(all_inputs, self._num_units, True, False,
                                     scope="cell"))
            cingate = 1.0 - cumsoftmax(linear(all_inputs, self._num_units, True, False,
                                     scope="cingate"))
            cforgetgate = cumsoftmax(linear(all_inputs, self._num_units, True, False,
                                     scope="cforgetgate"))

            overlap = cforgetgate * cingate
            forgetgate = forgetgate * overlap + (cforgetgate - overlap)
            ingate = ingate * overlap + (cingate - overlap)
            cy = forgetgate * state + ingate * cell
            new_state = outgate * tf.tanh(cy)
